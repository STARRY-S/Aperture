...
int ap_camera_get_view_matrix(mat4 *temp)
{
    ...
    glm_vec3_add(camera_using->position, camera_using->front, cameraTarget);
    glm_lookat(camera_using->position, cameraTarget, camera_using->up, *temp);
    return 0;
}
...
int ap_camera_process_key(int direction, float delta_time)
{
    ...
    switch (direction)
    {
        case AP_CAMERA_FORWARD:
        {
            ...
            glm_vec3_add(camera_using->position, temp, camera_using->position);
            break;
        }
        case AP_CAMERA_MOV_BACKWARD:
        {
            glm_vec3_scale(camera_using->front, velocity, temp);
            glm_vec3_sub(camera_using->position, temp, camera_using->position);
            break;
        }
